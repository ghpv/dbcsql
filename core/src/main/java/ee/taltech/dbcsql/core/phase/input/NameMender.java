...
public class NameMender
{
	...
	public void setVerifySQLKeywords(boolean b)
	{
	...
	public String mendName(String name) throws NameMenderException
	{
		...
		this.name = name;
		this
			.translateAllEstonianChars()
			.spaceToUnderscore()
			.removeExtra()
			.changeCamelCaseToSnakeCase()
			.changeToLowercase()
			.verifyNonEmpty()
			.verifyPrefixIsLowercaseLatin()
			.verifyDoesNotStartWithDigit()
			.prependPrefix()
			.verifyNotKeyword()
		;
		return this.name;
	...
	private NameMender translateAllEstonianChars()
	{
		this.name = this
			.name
			.replaceAll("Õ", "O")
	...
	private NameMender spaceToUnderscore()
	{
		this.name = this
			.name
			.replaceAll("\s", "_")
	...
	private NameMender changeCamelCaseToSnakeCase()
	{
		this.name = Pattern
			.compile("([a-z])([A-Z])")
			.matcher(this.name)
	...
	private NameMender changeToLowercase()
	{
		this.name = this
			.name
			.toLowerCase()
	...
	private NameMender removeExtra()
	{
		this.name = this
			.name
	...
	private NameMender verifyNonEmpty()
	{
		if (this.name.isBlank())
		{
			throw new NameMenderException("Name is empty");
		}
		return this;
	}

	private NameMender verifyDoesNotStartWithDigit()
	{
		if (this.hasPrefix())
		{
			return this;
	...
	private NameMender verifyPrefixIsLowercaseLatin()
	{
		if (this.hasPrefix()
			&& Pattern.compile(".*[^a-z].*").matcher(this.prefix).matches())
		{
			throw new NameMenderException("Prefix may not contain anything other than [a-z]");
		}
		return this;
	}
	...
	private NameMender prependPrefix()
	{
		if (this.hasPrefix() && !this.isSQLKeyword())
		{
